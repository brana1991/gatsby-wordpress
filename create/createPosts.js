const {
  ...
  BlogPreviewFragment,
} = require("../src/templates/posts/data");
...
const categories = require.resolve("../src/templates/posts/categories.js");
const { blogURI } = require("../global-variable");
...
module.exports = async ({ actions, graphql }) => {
  ...
  const fetchCategories = async variables => {
    const result = await graphql(GET_CATEGORIES, variables);
    const {
      wpcontent: {
        categories: { nodes },
      },
  ...
  categoryPages &&
    categoryPages.map((category, index) => {
      console.log(`create categoryPage: ${category.uri}`);
      createPage({
        path: `${category.uri}`,
